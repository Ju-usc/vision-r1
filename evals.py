...
# usage response = llm_eval_score(input_xml, output_xml, ex_cookies)
...
# if __name__ == "__main__":
#     parser = argparse.ArgumentParser(description="Add raw per-ingredient / per-step vectors to recipes")
#     parser.add_argument("--xml_prediction",  type=Path, default=Path("data/recipe_pairs.jsonl"))
#     parser.add_argument("--input_golden", type=Path, default=Path("data/recipe_pairs_raw_vec.jsonl"))
#     parser.add_argument("--image_path",     type=str,  default="all-MiniLM-L6-v2")
#     args = parser.parse_args()

#     with open(args.xml_prediction, 'r', encoding='utf-8') as f:
#         xml_pred = f.read()
#     with open(args.xml_golden, 'r', encoding='utf-8') as f:
#         csv_gold = f.read()
...
def compute_evals(model_output_xml, input_data_example):
    input_example_xml = input_data_example["recipe_xml"]
    image_path = input_data_example["image_path"]

    pred_recipe = parse_recipe_xml(model_output_xml)
    golden_recipe = parse_recipe_xml(input_example_xml)
    if pred_recipe == None: return None
    # print(truth_dict)
    golden_recipe['parsed_ingredients'] = golden_recipe['ingredients']
    golden_recipe['instruction_steps'] = golden_recipe['steps']
    golden_recipe['ingredients_embeddings'] = input_data_example['ing_vecs']
    golden_recipe['instructions_embeddings'] = input_data_example['step_vecs']
    ...
    print("Calculating LLM evaluation...")
    llm_evaluation = llm_eval_score(input_data_example["recipe_xml"], model_output_xml, image_path)

    start = llm_evaluation.find('{')
    end = llm_evaluation.rfind('}')
    # json_string = llm_evaluation[8: -4]
